...
## draw graph from data variation
...
data <- data %>%
  mutate(raw_value = str_extract(conc, "^(\\d)(\\.)(\\d)+|^(\\d)+")) %>% # the order is important.
  mutate(unit = str_extract(conc, "(\\D)+$"))
data$raw_value = as.double(data$raw_value)

data.gemcitabine <- data[data$drug=="Gemcitabine",]
data.verteporfin <- data[data$drug=="Verteporfin",]
...
```{r}
plot(OD.value~raw_value,data.gemcitabine ,subset=concentration!=0,log="x")
# use log for x-axis
plot(OD.value~raw_value,data.verteporfin ,subset=concentration!=0,log="x")
```
...
analyze <- function(title, data, line.color) {
  model.LL4 <- drm(formula=OD.value~raw_value,
                   data=data,
  ...
  plot(OD.value~raw_value,data.gemcitabine ,subset=concentration!=0,log="x", add=TRUE)
...
analyze("Gemcitabine IC50", gemcitabine.summary, data.gemcitabine, "blue")
analyze("Verteporfin IC50", data.verteporfin, "blue")
```
